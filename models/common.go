...
// If the login provided is invalid, this will return
// nil, ErrNotFound
// If the password provided is invalid, this will return
// nil, ErrPasswordIncorrect
// If the login and password are both valid, this will return
// user, nil
// Otherwise if another error is encountered this will return
// nil, error
func (ug *userGorm) Authenticate(login, password string) (*User, error) {
	foundUser, err := ug.ByLogin(login)
	if err != nil {
		return nil, err
	}
...
// ByLogin looks up a user with the given login and
// returns that user.
// If the user is found, we will return a nil error
// If the user is not found, we will return ErrNotFound
// If there is another error, we will return an error with
// more information about what went wrong. This may not be
// an error generated by the models package.
func (ug *userGorm) ByLogin(login string) (*User, error) {
	var user User
	db := ug.db.Where("login = ?", login)
	err := first(db, &user)
	return &user, err
}
...
// валидатор также вмещает в себя нормализацию (подготовка кода к хранению в дб в одном формате)
...
// Чекает, что login и пароль не пустые
// Если введены только одни пробелы (насчет пробельных символов не уверен), то это формально не считается пустой строкой
// Имеется нормализация в ByLogin логина, кт как раз триммирует пробелы, она происходит после валидации
// Но для пароля такой нормализации нет и не будет, т.к. пробел может быть частью пароля
// Поэтому, нужно чекать на непустые логин И пароль и далее нормализировать (триммировать) только логин
func (uv *userValidator) Authenticate(login, password string) (*User, error) {
	user := User{
		Login:    login,
		Password: password,
	}
	...
	return uv.UserDB.Authenticate(user.Login, user.Password)
}
...
// ByLogin will normalize an login address before passing
// it on to the database layer to perform the query.
func (uv *userValidator) ByLogin(login string) (*User, error) {
	user := User{
		Login: login,
	}
	...
	return uv.UserDB.ByLogin(user.Login)
}
...
func (uv *userValidator) loginRequired(user *User) error {
	if user.Login == "" {
...
// насчет 1-го if-a смотри стр 425
...
func (uv *userValidator) normalizeLogin(user *User) error {
...
// по сути это нормализация пароля
